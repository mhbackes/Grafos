...
#reader(lib "htdp-advanced-reader.ss" "lang")((modname trabalhografos) (read-case-sensitive #t) (teachpacks ((lib "convert.ss" "teachpack" "htdp"))) (htdp-settings #(#t constructor repeating-decimal #t #t none #f ((lib "convert.ss" "teachpack" "htdp")))))
...
(define (remove* lista)
  (cond
    [(empty? lista) empty]
    [(false? (first lista)) (remove* (rest lista))]
    [else (cons (first lista) (remove* (rest lista)))]))

(define (dígrafo-tem-cíclo? grafo)
    (cond
      [(empty? (remove* (map (lambda (x) (busca-larg (rest x) empty (first x) grafo)) grafo))) false]
      [else true]))
...
(define (pseudografo-tem-cíclo? grafo)
    (cond
      [(tem-loop? grafo) true] 
      [else 
      (ormap (lambda (x) (pseudo-ciclo (rest x) (first x) grafo)) grafo)]))


(define (tem-ciclo? grafo tamanho)
  (cond
    [(é-dígrafo? grafo tamanho) (imprime-tem-ciclo-di grafo)]
    [else (imprime-tem-ciclo-pseudo grafo)]))

(define (imprime-tem-ciclo-pseudo grafo)
    (cond
      [(pseudografo-tem-cíclo? grafo) (printf "O grafo possui ciclo.\n")]
      [else (printf "O grafo não possui ciclo.\n")]))

(define (imprime-tem-ciclo-di grafo)
    (cond
      [(dígrafo-tem-cíclo? grafo) (printf "O grafo possui ciclo.\n")]
      [else (printf "O grafo não possui ciclo.\n")]))
